...
data=np.array(
[
    [0.1,   7.1,    -1],#このデータがあると線形分離できない
    [0.3,   3.9,    -1],#このデータがあると線形分離できない
    [0.4,   4.9,    -1],#このデータがあると線形分離できない
    [1,     1.1,    -1],
    [1,     3.9,    1],
    [1.5,   4.9,    1],#上が1
    [2,     1.9,    -1],
...
def E(w):
    ...
    for i in range(len(X)):
        #e += max(0,-t[i]*(y[i]-(w[0]*X[i]+w[1])))
        #e += max(0,-t[i]*(y[i]-(X[i]**2+w[0]*X[i]+w[1])))
...
#y_pred = X_pred**2 + w[0]*X_pred + w[1]
...
while True:
    x1 = float(input("x1の値は？"))
    x2 = float(input("x2の値は？"))
    if predict(x1,x2,lambda x: x**2 + w[0]*x + w[1])==1:
        print("この点はA型（ラベル=1）です。")
    elif predict(x1,x2,lambda x: x**2 + w[0]*x + w[1])==-1:
        print("この点はB型（ラベル=-1）です。")
    elif predict(x1,x2,lambda x: x**2 + w[0]*x + w[1])==0:
        print("この点は決定境界線上にあります。")
    print("")